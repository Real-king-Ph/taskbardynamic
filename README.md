...
本版本集中修复 PrimoCache 采样就绪状态、外部命令解析健壮性与绘图窗口时钟问题，已作为正式版发布（GitHub Release `v1.2.2`，Latest，标签指向 `4e0c900`）。

**缺陷修复**

- 修复启动或恢复显示后速率尚未就绪时仍可能显示 `0.00B/s` 或空闲前旧值的问题；首个采样间隔现在显示 `--`，完成两次采样后恢复数值；
- 修复 `rxpcc` 输出中非法或缺失的 `Total Read` / `Cached Read` 仍可能被当成有效零值的问题；字段必须完整且可解析；
- 为单个累计计数及多卷累加增加 `uint64_t` 溢出检查，异常输出会直接令本次采样失败；
- 绘图滑动窗口改用 `std::chrono::steady_clock` 单调时钟，避免系统时间大幅调整影响历史极值；
- 修正 `DynamicInfo` 中 `lable_` 的拼写为 `label_`。
...
本版本是 PrimoCache 显示项的缺陷修复与健壮性改进，已作为正式版发布（GitHub Release `v1.2.1`，标签指向 `261213e`）。
